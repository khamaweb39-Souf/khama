    ...
    >
      ...
      <div className="p-4 flex-1 flex flex-col space-y-2">
        <div className="flex items-center justify-between text-[10px] text-gray-400">
          <span className="bg-gray-50 px-2 py-0.5 rounded-full">{product.category?.name?.ar || 'تصنيف'}</span>
        ...
        <div className="flex items-center gap-1">
           ...
           {product.store?.isVerified && <ShieldCheck className="w-3 h-3 text-blue-500" />}
        </div>

        <div className="mt-auto pt-2 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-lg font-black text-burgundy leading-none">
              {product.price?.toLocaleString()} <span className="text-[10px]">دج</span>
            </span>
