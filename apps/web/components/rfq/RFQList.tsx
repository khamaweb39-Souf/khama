...
import { 
  ...
  ChevronLeft, Filter, Search, MoreHorizontal
} from 'lucide-react';
      ...
      <div className="grid gap-6">
        {ACTIVE_RFQS.map((rfq) => (
          ...
          >
            ...
            {rfq.urgent && <div className="absolute top-0 right-0 w-2 h-full bg-red-500/80" />}
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6">
              <div className="flex-1 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-muted font-medium">
                    <span className="flex items-center gap-1"><Briefcase className="w-3.5 h-3.5" /> {rfq.buyer}</span>
                    <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {rfq.location}</span>
                    <span className="flex items-center gap-1"><Layers className="w-3.5 h-3.5" /> {rfq.category}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
                   <div>
                     ...
                     <p className="text-sm font-bold text-charcoal">{rfq.quantity}</p>
                   </div>
                   <div>
                     <p className="text-[10px] text-muted uppercase mb-1">التركيب</p>
                     <p className="text-sm font-bold text-charcoal">{rfq.composition}</p>
                   </div>
                   <div>
                     <p className="text-[10px] text-muted uppercase mb-1">الوزن</p>
                     <p className="text-sm font-bold text-charcoal">{rfq.gsm}</p>
                   </div>
                   <div>
                     <p className="text-[10px] text-muted uppercase mb-1">العروض المستلمة</p>
                     <p className="text-sm font-bold text-accent">{rfq.responses} عروض</p>
                   </div>
                </div>
              </div>
              ...
              <div className="flex flex-col justify-between items-end gap-6 md:min-w-[180px]">
                <div className="text-left">
                  <p className="text-[10px] text-muted uppercase mb-1">الموعد النهائي</p>
                  <p className="text-lg font-bold text-charcoal">{rfq.deadline}</p>
                </div>
