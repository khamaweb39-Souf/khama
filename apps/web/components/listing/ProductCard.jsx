...
import { Star, MapPin, ShieldCheck, Layers } from 'lucide-react';
  ...
  return (
    <div className="relative group">
      ...
      <button 
        ...
        className={`
          absolute top-3 left-3 z-20 p-2 rounded-xl transition-all duration-300 shadow-lg backdrop-blur-md
          ${isCompared ? 'bg-gold text-charcoal scale-110 shadow-gold/20' : 'bg-white/60 text-charcoal opacity-0 group-hover:opacity-100 hover:bg-white'}
        `}
        title={isCompared ? 'إزالة من المقارنة' : 'إضافة للمقارنة'}
      >
        <Layers className={`w-4 h-4 ${isCompared ? 'animate-pulse' : ''}`} />
      </button>

      <Link 
        href={`/products/${product.id}`}
        className="block bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-burgundy/5 transition-all duration-500 flex flex-col h-full"
      >
        {/* صورة المنتج */}
